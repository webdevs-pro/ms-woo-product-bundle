...
if ( ! class_exists( 'MSleverWoosb' ) && class_exists( 'WC_Product' ) ) {
	class MSleverWoosb {
		...
		function woosb_admin_menu() {
			add_submenu_page( 'wpclever', esc_html__( 'MS Product Bundles', 'woo-product-bundle' ), esc_html__( 'Product Bundles', 'woo-product-bundle' ), 'manage_options', 'wpclever-woosb', array(
				&$this,
		...
		function woosb_admin_menu_content() {
            ...
            <div class="wpclever_settings_page wrap">
                <h1 class="wpclever_settings_page_title"><?php echo esc_html__( 'MS Product Bundles', 'woo-product-bundle' ) . ' ' . WOOSB_VERSION; ?></h1>
                                    ...
                                    <td>
                                        <select name="_woosb_variations_selector">
                                            ...
                                            <option
												...
												<?php esc_html_e( 'Use MS Variations Radio Buttons', 'woo-product-bundle' ); ?>
                                            </option>
                                        </select> <span class="description">If you choose "Use MS Variations Radio Buttons", please install <a
                                                    ...
                                                    class="thickbox"
                                                    title="Install MS Variations Radio Buttons">MS Variations Radio Buttons</a> to make it work.</span>
                                    </td>
                                </tr>
                                <tr>
                                        ...
                                        <select name="_woosb_bundled_link">
                                            ...
                                            <option
												...
												<?php esc_html_e( 'No', 'woo-product-bundle' ); ?>
                                                    ...
                                                    class="thickbox" title="Install MS Smart Quick View">MS Smart Quick View</a> to make it work.</span>
                                    </td>
                                </tr>
                                <tr>
		...
		function woosb_add_to_cart_validation( $passed, $product_id ) {
			...
			if ( $_product && $_product->is_type( 'woosb' ) && ( $items = $_product->get_items() ) ) {
				if ( isset( $_POST['woosb_ids'] ) && ( $_product->is_optional() || $_product->has_variables() ) ) {
					$ids   = MSleverWoosb_Helper::woosb_clean_ids( $_POST['woosb_ids'] );
		...
		function woosb_add_to_cart_items( $items, $cart_item_key, $product_id, $quantity ) {
			...
			if ( is_array( $items ) && ( count( $items ) > 0 ) ) {
				foreach ( $items as $item ) {
					...
					$_variation_id = 0;
					$_variation    = array();
					$_price        = MSleverWoosb_Helper::woosb_get_price( $_product, 'min', $discount );
				...
				if ( $product && $product->is_type( 'woosb' ) && ( $items = $product->get_items() ) ) {
					...
					foreach ( $items as $item ) {
						...
						if ( $fixed_price ) {
							...
							$_price = (float) ( 100 - $discount_percentage ) * MSleverWoosb_Helper::woosb_get_price( $_product, 'min', true ) / 100;
		...
		function woosb_get_search_results() {
			...
			$added_ids = explode( ',', MSleverWoosb_Helper::woosb_clean_ids( $_POST['ids'] ) );
		...
		function woosb_product_data_panels() {
			...
			$ids = MSleverWoosb_Helper::woosb_clean_ids( $ids );
		...
		function woosb_product_data_li( $product, $qty = 1, $search = false ) {
			...
			if ( class_exists( 'MSleverWoopq' ) && ( get_option( '_woopq_decimal', 'no' ) === 'yes' ) ) {
				$step = '0.000001';
			} else {
				$step = 1;
			}
			...
			$price     = MSleverWoosb_Helper::woosb_get_price( $product, 'min', false );
			$price_max = MSleverWoosb_Helper::woosb_get_price( $product, 'max', false );
		...
		function woosb_save_option_fields( $post_id ) {
			if ( isset( $_POST['woosb_ids'] ) && ! empty( $_POST['woosb_ids'] ) ) {
				update_post_meta( $post_id, 'woosb_ids', MSleverWoosb_Helper::woosb_clean_ids( $_POST['woosb_ids'] ) );
			}
				...
				if ( ! $product->is_fixed_price() ) {
					...
					if ( $product->is_optional() ) {
						...
						foreach ( $items as $item ) {
							...
							if ( $_product ) {
								$prices[] = MSleverWoosb_Helper::woosb_get_price_to_display( $_product, 1, 'min', $discount );
							}
						}
						...
						foreach ( $items as $item ) {
							...
							if ( $_product ) {
								$min_price += MSleverWoosb_Helper::woosb_get_price_to_display( $_product, $item['qty'], 'min', $discount );
								$max_price += MSleverWoosb_Helper::woosb_get_price_to_display( $_product, $item['qty'], 'max', $discount );
							}
						}
						...
						foreach ( $items as $item ) {
							...
							if ( $_product ) {
								$_price = MSleverWoosb_Helper::woosb_get_price_to_display( $_product, $item['qty'], 'min', $discount );
                ...
                <div class="woosb-products"
					...
					foreach ( $items as $item ) {
                        ...
                        <div class="woosb-product"
                             ...
                             data-price="<?php echo esc_attr( MSleverWoosb_Helper::woosb_get_price_to_display( $_product, 1, 'min', $discount ) ); ?>"
                            ...
                            <div class="woosb-title">
								...
								if ( $_product->is_in_stock() ) {
									echo '<input type="checkbox" class="woosb_item_checkbox" id="woosb_item_checkbox_' . $item['id'] . '" checked>';
								} else {
									echo '<input type="checkbox" id="optional_' . $item['id'] . '" disabled>';
								}
							...
							<?php if ( get_post_meta( $product_id, 'woosb_optional_products', true ) === 'on' ) {
								...
								if ( $_product->is_in_stock() ) {
									echo '<div style="display: none;" data-qty="' . $_qty . '" class="woosb-qty' . ( get_option( '_woosb_plus_minus', 'no' ) === 'yes' ? ' woosb-qty-plus-minus' : '' ) . '">';
                                ...
                                <div class="woosb-price">
                                    <div class="woosb-price-ori">
										...
										if ( ! $product->is_fixed_price() && ( $discount_percentage = $product->get_discount() ) ) {
											$show_new_price = true;
											$_product_price = MSleverWoosb_Helper::woosb_get_price( $_product, 'min', true ) * ( 100 - $discount_percentage ) / 100;
										} else {
											$show_new_price = false;
											$_product_price = MSleverWoosb_Helper::woosb_get_price( $_product, 'min', false );
										}

										switch ( get_option( '_woosb_bundled_price', 'price' ) ) {
											case 'price':
												if ( $show_new_price ) {
													$_price = wc_format_sale_price( MSleverWoosb_Helper::woosb_get_price_to_display( $_product, 1, 'min', $discount ), wc_get_price_to_display( $_product, array( 'price' => $_product_price ) ) );
												} else {
											...
											case 'subtotal':
												if ( $show_new_price ) {
													$_price = wc_format_sale_price( MSleverWoosb_Helper::woosb_get_price_to_display( $_product, $item['qty'], 'min', $discount ), wc_get_price_to_display( $_product, array(
...
function MSleverWoosb() {
	return MSleverWoosb::instance();
}